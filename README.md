...
Откройте в браузере **http://localhost:8000** — появится главное меню: «Играть» (имя пилота можно задать в «Настройках»), «Таблица лидеров» (топ-10 лучших результатов). Кнопка «?» в игре — справка с правилами. При первом входе запускается **интерактивный онбординг**: он подсвечивает панели и просит реально выбрать заказ, ровер и запустить доставку.

Быстрый старт в Windows: двойной клик по `start.bat`.

### Десктоп-версия (нативное окно, без браузера)
...
Запускает тот же сервер в потоке на свободном локальном порту и открывает игру в собственном окне (WebView2 на Windows). Закрытие окна останавливает игру.

Сохранение — автоматическое в `data/*.json`. Новая игра — кнопка «Новая игра» или `POST /api/cmd {"cmd":"reset"}` (`?noMenu=1` в адресе — пропустить главное меню).

---

## Скриншоты

| Главное меню | Игровой экран (карта Луны, заказы, роверы) |
|---|---|
| ![Главное меню](screenshots/menu.png) | ![Игровой экран](screenshots/game.png) |
...
7. Кнопка ▶ игра = пауза → ×5 ускорение: одно нажатие — запустить, второе — ускорить, третье — пауза. «Новая игра» — сброс.
...
- **Верфь**: витрина из 4 случайных моделей — грузоподъёмность, батарея, скорость, цена и требуемое число доставок генерируются с ростом вашего прогресса. Каждые 35–70 игровых минут один слот обновляется, после покупки витрина пополняется новой моделью. Купленные роверы получают уникальные id (можно покупать повторно разные экземпляры).
...
- Полностью самодостаточная браузерная игра: канвас-карта Луны (случайный мир: зоны, кратеры, точки заказов, роверы с кольцом батареи, ночь/день), панели заказов и роверов, оценка миссии, журнал событий, справка, экран финала, главное меню с настройками и таблицей лидеров.
- Бэкенд — Python stdlib (HTTP + фоновая симуляция + хранилище JSON).
- Сгенерировано opencode (AI-ассистент): архитектура, вся логика симуляции, интерфейс и текст. Числа баланса подобраны вручную и проверены прогоном сценариев (вес, батарея, буря, застрявший ровер, game over). Случайная генерация мира (зоны-мозаика, кратеры) — собственная процедурная логика.
