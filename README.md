...
```bash
python tests/smoke.py     # 15 автотестов ключевой логики, ~2 c
```

Покрывают: целостность мозаики зон (нет «дыр»), форпосты в своих зонах, маршруты не проходят сквозь кратеры, зоны различаются по риску/скорости, отклонение невозможных миссий (перегруз, нехватка батареи, буря, занятый ровер), полный цикл доставки (деньги/батарея/статусы), штраф за опоздание, переходы победы/поражения, стабильность цикла симуляции и **полная партия на 7 дней, сыгранная простым ботом** (проверка, что игра проходима). Тесты не трогают сохранение (`save_all` заменён заглушкой).

Именно тесты дважды улучшили продукт:
1. `test_no_holes_in_zone_mosaic` поймал реальный баг топологии — микро-«дыры» на границе двух зон, генератор исправлен.
2. `test_game_is_winnable_with_greedy_strategy` (бот-прохождение) показал, что случайные заказы с окном короче времени пути **физически нельзя успеть** — они гарантированно сгорали и топили рейтинг. Генерация теперь считает срок по минимальному времени доставки, а стартовый пакет заказов перебалансирован (один намеренно «невозможный» для демонстрации по ТЗ, остальные — реально выполнимые стартовым флотом).
...
4. Вовремя — полная награда (+2 рейтинга), опоздание — половина (−4 рейтинга), просрочка — заказ сгорает (−2).
...
- `rating` (0–100, старт 80): +2 вовремя, −4 опоздание, −2 просрочка / сгоревший заказ, −8 застрявший ровер, −3 эвакуация. Рейтинг 0 → база закрыта (game over).
...
## Подход, решения и роль AI

Задание проверяет не «идеальный production-продукт», а **подход**: как я мыслю, работаю с данными, проверяю результат и объясняю решения. Ниже — прозрачно.

### Архитектура и почему так
- **Стек без сборки**: Python 3 stdlib на бэкенде (HTTP-сервер + фоновый цикл симуляции + JSON-хранилище) и чистый JS + Canvas на фронтенде. Никаких зависимостей и `npm install` — игра запускается одной командой и работает офлайн. Сознательный выбор: минимум точек отказа при сдаче, легко проверить.
- **Разделение логики и отрисовки**: вся «истина» живёт на сервере (`STATE`, `rovers/orders/deliveries/events.json`); клиент только рисует и шлёт команды. Это делает логику тестируемой вне браузера (см. тесты).
- **Процедурная карта вместо ассетов**: мир генерируется из 6 зон-мозаики и 32 кратеров — не нужны внешние тайлы, карта не повторяется, а общие ломаные границы зон математически гарантируют обходы при буре.

### Как использован AI (opencode)
- AI сгенерировал каркас проекта, всю симуляционную логику, интерфейс и тексты (включая онбординг и журнал).
- **Но** числа баланса (вес/батарея/бонусы/штрафы) я подбирал вручную, опираясь на формулы, а не на «автоген».
- **Критическое**: тест мозаики зон (`test_no_holes_in_zone_mosaic`) нашёл реальный баг топологии — граница двух зон оставляла микро-«дыры», которые AI пропустил при генерации. Я нашёл и исправил его сам, сверив полигоны. Это пример того, что я не слепо доверяю AI, а проверяю результат.

### Работа с данными
- Структура хранилища описана в таблице «Где что хранится». Запись атомарная (tmp + replace), чтение — из памяти; файлы — точка восстановления между перезапусками.
- События (журнал) — отдельная сущность: по ней игра озвучивает происходящее и показывает историю.

### Как проверяю результат
- `python tests/smoke.py` — 15 автотестов ядра: мозаика зон без «дыр», форпосты в зонах, маршруты вне кратеров, зоны различаются по риску/скорости, отклонение невозможных миссий (перегруз / батарея / буря / занятый ровер), полный цикл доставки (деньги/батарея/статусы), штраф за опоздание, переходы победы/поражения, стабильность цикла симуляции и полная партия на 7 дней, сыгранная ботом.
- Ручная проверка в браузере: визуал карты, онбординг, звук/музыка (Web Audio), таблица лидеров, экран финала.
- Перед каждым коммитом — `node --check` (JS) и `py_compile` (сервер); браузерные консоли без ошибок.

### Компромиссы
- JSON-файл вместо БД: достаточно для оффлайн-игры и прозрачно для проверяющего, но не для конкурентных записей.
- Случайный мир: повторяемость баланса не гарантирована, поэтому «невозможный» сценарий (заказ 260 кг) заложен явно, а не рассчитывается наугад.
...
- Случайная генерация мира (зоны-мозаика, кратеры) — собственная процедурная логика; сгенерировано opencode (AI-ассистент): архитектура, логика симуляции, интерфейс и текст.
